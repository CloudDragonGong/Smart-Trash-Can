...
class ProcessChat:
    ...
    def match_mode(self, input_text):
        prompt = f"你是智能垃圾桶的语音助手，现在垃圾桶的情况是这样的：垃圾桶的满载情况：{self.data['full_load']} True是满载，False是未满载, 分别对应 " \
                 f"其他垃圾，厨余垃圾，可回收垃圾，有害垃圾 " \
                 f"。你现阶段的任务是这样的：你身为垃圾桶可以完成下面四种模式，你通过猜测用户的说话的含义，将接下来对话的模式分类，分为：1.分类模式（用户请求你将少量的垃圾进行分类），2" \
                 f".成堆投放模式（用户连续投放很多垃圾，或者是一次性投放很多很多垃圾），3" \
                 f".取垃圾桶模式（就是用户明确想请求把垃圾桶取出来然后倒掉里面的垃圾,比如我要取出某个垃圾桶然后倒掉，注意，用户问你那个垃圾桶满载的时候，并不是去取垃圾桶模式，而是聊天模式）4"
        ".聊天模式（不是上面3"
        "种模式的话就是聊天模式用户可能会和你闲聊，用户可能还会询问你一些垃圾桶系统的一些状况，例如垃圾桶的满载情况，哪个垃圾桶装满了？或者投放频率啥的，说明用户再和你闲聊，或者询问情况，这些都是聊天模式，你就回答聊天模式）"
        "如果用户想投放的垃圾的种类所对应的垃圾桶已经满载了，说明已经不能投放垃圾了，你就回答这是聊天模式，因为方便我后面程序的处理。"
        "你直接回答你预测出来的模式，不用解释原因，"
        model = Model(prompt)
